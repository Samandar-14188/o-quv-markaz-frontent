...
export default function Payments() {
  ...
  const [form, setForm] = useState({
    student_id: '', amount: '', month: MONTHS[new Date().getMonth()],
    year: new Date().getFullYear(), payment_date: new Date().toISOString().split('T')[0],
    status: 'paid', note: ''
  });
  ...
  const filtered = payments.filter(p => {
    ...
    return name.includes(search.toLowerCase()) || p.month.toLowerCase().includes(search.toLowerCase());
  });

  const handleStudentChange = (id) => {
    const s = students.find(s => s.id === parseInt(id));
    setForm(f => ({ ...f, student_id: id, amount: s ? s.monthly_fee : f.amount }));
  };

  const handleSave = async () => {
    if (!form.student_id || !form.amount) return toast.error("Student va miqdor kerak!");
    try {
    ...
    <div>
      <div className="sticky top-0 z-40 bg-[#111827] border-b border-[#1e2d45] px-8 py-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-0.5">Jami: {filtered.length} ta</p>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={() => { setForm({ student_id:'', amount:'', month: MONTHS[new Date().getMonth()], year: new Date().getFullYear(), payment_date: new Date().toISOString().split('T')[0], status:'paid', note:'' }); setModal(true); }}
            ...
            {tabs.map(t => (
              <button key={t.key} onClick={() => setFilter(t.key)}
                className={`px-4 py-2 text-sm font-semibold rounded-t-lg border-b-2 transition
                  ${filter === t.key ? 'text-blue-400 border-blue-400' : 'text-slate-400 border-transparent hover:text-white'}`}>
                {t.label}
              </button>
            ))}
          </div>
          <table className="w-full">
            <thead>
              <tr className="bg-white/[0.02]">
                {['#', "O'quvchi", 'Kurs', 'Oy', 'Miqdor', 'Sana', 'Holat', ''].map(h => (
                  <th key={h} className="text-left px-4 py-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wide border-b border-[#1e2d45]">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={8} className="text-center py-12 text-slate-500">📭 To'lov topilmadi</td></tr>
              ) : filtered.map((p, i) => (
                <tr key={p.id} className="border-b border-[#1e2d45]/50 hover:bg-blue-500/5 transition">
                  ...
                  <td className="px-4 py-3">
                    <span className="bg-blue-500/10 text-cyan-400 text-[11px] px-2 py-1 rounded-md">{p.course_name || '—'}</span>
                  </td>
                  <td className="px-4 py-3 text-sm">{p.month} {p.year}</td>
                  <td className="px-4 py-3 font-mono text-sm font-bold text-green-400">+{fmt(p.amount)}</td>
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full
                      ...
                      {p.status === 'paid' ? '✅ To\'langan' : p.status === 'overdue' ? '⚠️ O\'tgan' : '⏳ Kutilmoqda'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <button onClick={() => handleDelete(p.id)} className="text-xs bg-[#1a2235] text-slate-400 hover:text-red-400 border border-[#1e2d45] px-2.5 py-1.5 rounded-lg">🗑</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#111827] border border-[#1e2d45] rounded-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1.5">O'quvchi</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1.5">Holat</label>
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase mb-1.5">Izoh</label>
                <input value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} placeholder="Ixtiyoriy..."
                  className="w-full bg-[#1a2235] border border-[#1e2d45] text-white rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-blue-500" />
              </div>
            </div>
